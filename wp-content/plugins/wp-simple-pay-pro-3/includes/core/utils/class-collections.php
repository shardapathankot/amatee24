	...
	public function add( $collection_id, $collection ) {
		if ( $this->has_item( $collection_id ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'The %s collection already exists and could not be added.', 'simple-pay' ),
					$collection_id
				)
			);
		}
		...
		if ( ! is_a( $collection, '\SimplePay\Core\Utils\Collection' ) ) {
			return new \WP_Error(
				...
				sprintf(
					/* translators: %s Collection ID that could not be registered. */
					__( 'The %s collection must be an instance of \SimplePay\Core\Utils\Collection.', 'simple-pay' ),
					$collection_id
				)
			);
		}
