	...
	<?php if ( false === $has_config ) : ?>
		<div class="simpay-license-field">
			...
			<?php if ( 'valid' !== $license->get_status() ) : ?>
				...
				>
					<?php esc_html_e( 'Activate', 'simple-pay' ); ?>
				</button>
		...
		<div class="simpay-license-message simpay-license-message--<?php echo esc_html( $license->is_valid() ? 'valid' : 'invalid' ); ?>">
			...
			<a href="<?php echo esc_url( $refresh_url ); ?>">
				<?php esc_html_e( 'Refresh Key', 'simple-pay' ); ?>
			</a>
		</div>
		<?php endif; ?>
	<?php else : ?>
		<p>
			<?php
				echo wp_kses(
					__( 'Your license key is globally defined via <code>SIMPLE_PAY_LICENSE_KEY</code> set in <code>wp-config.php</code>. It cannot be modified from this screen.', 'simple-pay' ),
					array(
						'code' => array(),
					)
				);
			...
			<a href="<?php echo esc_url( $refresh_url ); ?>">
				<?php esc_html_e( 'Refresh license data.', 'simple-pay' ); ?>
			</a>
		</p>
	<?php endif; ?>
