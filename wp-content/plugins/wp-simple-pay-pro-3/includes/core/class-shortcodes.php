				...
				return wp_kses_post(
					wpautop(
						sprintf(
							...
							__( 'This payment form is currently unpublished. %1$sPreview payment form →%2$s.', 'simple-pay' ),
				...
				if ( empty( $prices ) ) {
					if ( current_user_can( 'manage_options' ) ) {
						...
						return wp_kses_post(
							sprintf(
								/* translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
								__( 'Attention: The payment form does not have prices configured. %1$sAdd price options%2$s to collect payments.', 'simple-pay' ),
