...
class SMTP {
	 ...
	 * @since 4.3.0
	 *
	 * @param \SimplePay\Core\Settings\Subsection_Collection<\SimplePay\Core\Settings\Subsection> $subsections
	 *                                                                                            Subsections.
	 * @return void
	 */
	 ...
	 * @since 4.3.0
	 *
	 * @param \SimplePay\Core\Settings\Setting_Collection<\SimplePay\Core\Settings\Setting> $settings
	 *                                                                                      Settings.
	 * @return void
	 */
	...
	private function get_js_strings() {
		...
		$error_could_not_install = sprintf(
			wp_kses( /* translators: %s - Lite plugin download URL. */
				__( 'Could not install the plugin automatically. Please <a href="%s">download</a> it and install it manually.', 'simple-pay' ),
				array(
					'a' => array(
						'href' => true,
					),
		...
		$error_could_not_activate = sprintf(
			wp_kses( /* translators: %s - Lite plugin download URL. */
				__( 'Could not activate the plugin. Please activate it on the <a href="%s">Plugins page</a>.', 'simple-pay' ),
				array(
					'a' => array(
						'href' => true,
					),
		...
		return array(
			...
			'installing'               => esc_html__( 'Installing...', 'simple-pay' ),
			'activating'               => esc_html__( 'Activating...', 'simple-pay' ),
			'activated'                => esc_html__( 'WP Mail SMTP Installed & Activated', 'simple-pay' ),
			'install_now'              => esc_html__( 'Install Now', 'simple-pay' ),
			'activate_now'             => esc_html__( 'Activate Now', 'simple-pay' ),
			'download_now'             => esc_html__( 'Download Now', 'simple-pay' ),
			'plugins_page'             => esc_html__( 'Go to Plugins page', 'simple-pay' ),
			...
			'smtp_settings'            => esc_html__( 'Go to SMTP settings', 'simple-pay' ),
			'smtp_wizard'              => esc_html__( 'Open Setup Wizard', 'simple-pay' ),
	...
	private function output_section_heading() {
		...
		printf(
			...
			esc_attr__( 'WP Mail SMTP ♥ WP Simple Pay', 'simple-pay' ),
			esc_html__( 'Making Email Deliverability Easy for WordPress', 'simple-pay' ),
			esc_html__( 'WP Mail SMTP fixes deliverability problems with your WordPress emails so you can reliably send payment receipts, form notifications and more!', 'simple-pay' )
		);
	}
	...
	private function output_section_screenshot() {
		...
		printf(
			...
			esc_attr__( 'WP Mail SMTP screenshot', 'simple-pay' ),
			esc_url( SIMPLE_PAY_INC_URL . 'core/assets/images/smtp/screenshot-full.png' ),
			esc_html__( 'Improves email deliverability in WordPress.', 'simple-pay' ),
			esc_html__( 'Used by 2+ million websites.', 'simple-pay' ),
			esc_html__( 'Free mailers: SMTP.com, Sendinblue, Google Workspace/ Gmail, Mailgun, Postmark, SendGrid.', 'simple-pay' ),
			esc_html__( 'Pro mailers: Amazon SES, Microsoft 365/ Outlook.com, Zoho Mail.', 'simple-pay' )
		);
	}
	...
	private function output_section_step_setup() {
		...
		printf(
			...
			esc_url( SIMPLE_PAY_INC_URL . 'core/assets/images/smtp/' . $step['icon'] ),
			esc_attr__( 'Step 2', 'simple-pay' ),
			esc_html__( 'Set Up WP Mail SMTP', 'simple-pay' ),
			esc_html__( 'Select and configure your mailer.', 'simple-pay' ),
			esc_attr( $step['button_class'] ),
	...
	private function get_data_step_install() {
		...
		if (
			...
			$step['button_text']   = esc_html__( 'Install WP Mail SMTP', 'simple-pay' );
			$step['button_class']  = 'button-primary';
			$step['button_action'] = 'install';
			$step['plugin']        = $this->config['lite_download_url'];

			if ( ! current_user_can( 'install_plugins' ) ) {
				$step['heading']     = esc_html__( 'WP Mail SMTP', 'simple-pay' );
				$step['description'] = '';
				$step['button_text'] = esc_html__( 'WP Mail SMTP on WordPress.org', 'simple-pay' );
			...
			$step['button_text'] = $this->output_data['plugin_activated']
				? esc_html__( 'WP Mail SMTP Installed & Activated', 'simple-pay' )
				: esc_html__( 'Activate WP Mail SMTP', 'simple-pay' );
	...
	private function get_data_step_setup() {
		...
		if ( $this->output_data['plugin_activated'] ) {
			$step['section_class'] = '';
			$step['button_class']  = 'button-primary';
			$step['button_text']   = esc_html__( 'Open Setup Wizard', 'simple-pay' );
		} else {
			$step['section_class'] = 'grey';
			$step['button_class']  = 'grey disabled';
			$step['button_text']   = esc_html__( 'Start Setup', 'simple-pay' );
		}

		if ( $this->output_data['plugin_setup'] ) {
			$step['icon']        = 'step-complete.svg';
			$step['button_text'] = esc_html__( 'Go to SMTP settings', 'simple-pay' );
		}
	...
	public function ajax_check_plugin_status() {
		...
		if ( ! check_ajax_referer( 'simpay-admin', 'nonce', false ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'You do not have permission.', 'simple-pay' ),
				)
			);
		}
		...
		if ( ! $this->is_smtp_activated() ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Plugin unavailable.', 'simple-pay' ),
				)
			);
		}
