<?php
/**
 * Report: Sucessful Payments Period Over Period
 *
 * @package SimplePay
 * @subpackage Core
...
/**
 * SuccessfulPaymentsPeriodOverPeriodReport class.
 *
 ...
 * @since 4.6.7
 */
class SuccessfulPaymentsPeriodOverPeriodReport extends Report\AbstractReport implements SubscriberInterface {
	...
	public function get_report( $request ) {
		...
		$report = array(
			...
			'chart'           => array(
				...
				'datasets'      => array(
					array(
						'label' => __( 'Current period', 'simple-pay' ),
						'rgb'   => $primary_color,
						'data'  => $datasets[1],
						'type'  => 'line',
					),
					array(
						'label' => __( 'Previous period', 'simple-pay' ),
						'rgb'   =>
	...
	private function get_results( $dates, $range, $currency ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT {$select_date} as date, COUNT(*) as value FROM {$wpdb->prefix}wpsp_transactions WHERE livemode = %d AND currency = %s AND status = 'succeeded' AND object IN ('payment_intent', 'setup_intent') AND date_created BETWEEN %s AND %s GROUP BY date", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
