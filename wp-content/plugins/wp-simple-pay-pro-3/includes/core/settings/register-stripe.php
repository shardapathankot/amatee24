...
function register_section( $sections ) {
	...
	$sections->add(
		new Settings\Section(
			array(
				'id'       => 'stripe',
				'label'    => esc_html_x( 'Stripe', 'settings section label', 'simple-pay' ),
				'priority' => 20,
			)
		)
	);
	...
	$subsections->add(
		new Settings\Subsection(
			array(
				'id'       => 'account',
				'section'  => 'stripe',
				'label'    => esc_html_x( 'Account', 'settings subsection label', 'simple-pay' ),
				'priority' => 10,
			)
		)
	);
	...
	$subsections->add(
		new Settings\Subsection(
			array(
				'id'       => 'locale',
				'section'  => 'stripe',
				'label'    => esc_html_x( 'Locale', 'settings subsection label', 'simple-pay' ),
				'priority' => 20,
			)
		)
	);
...
function register_account_settings( $settings ) {
			...
			array(
				...
				'output'     => function() {
					...
					if ( simpay_check_keys_exist() ) {
						$html .= '<div id="simpay-stripe-account-info" class="simpay-stripe-account-info notice inline" data-nonce="' . wp_create_nonce( 'simpay-stripe-connect-information' ) . '"><p><span class="spinner is-active"></span> <em>' . esc_html__( 'Retrieving account information...', 'simple-pay' ) . '</em></p></div>';
					}
					...
					if ( false === simpay_get_account_id() || ! simpay_check_keys_exist() ) {
						...
						$html .= '<p id="simpay-stripe-auth-error-account-actions" style="display: none;">' . sprintf(
							...
							__( '%1$sdisconnect this account%2$s.', 'simple-pay' ),
						...
						$html .= '<p id="simpay-stripe-activated-account-actions" style="display: none;">' . sprintf(
							/* translators: %1$s Stripe payment mode. %2$s Opening anchor tag for reconnecting to Stripe, do not translate. %3$s Opening anchor tag for disconnecting Stripe, do not translate. %4$s Closing anchor tag, do not translate. */
							__( 'Your Stripe account is connected in %1$s mode. %2$sDisconnect this account%3$s.', 'simple-pay' ),
							'<strong>' . $mode . '</strong>',
							'<a href="' . esc_url( simpay_get_stripe_disconnect_url() ) . '" class="simpay-disconnect-link">',
							'</a>'
						) . '</p>';
						...
						$html .= sprintf(
							...
							esc_attr__( 'Are you sure?', 'simple-pay' ),
							esc_html__(
							...
							'<a href="' . esc_url( simpay_docs_link( 'Review the documentation', 'switching-stripe-accounts', 'stripe-account-settings', true ) ) . '" class="simpay-external-link" rel="noopener noreferrer" target="_blank">' . esc_html__( 'Review the documentation', 'simple-pay' ) . Utils\get_external_link_markup() . '</a>'
						);

						$html .= '<p id="simpay-stripe-unactivated-account-actions" style="display: none;"><a href="' . esc_url( simpay_get_stripe_disconnect_url() ) . '">' .
						__( 'Disconnect temporary account', 'simple-pay' ) .
						'</a></p>';
					}
	...
	// Keys.
	$keys = array(
		'test_publishable_key' => esc_html__( 'Test Publishable Key', 'simple-pay' ),
		'test_secret_key'      => esc_html__( 'Test Secret Key', 'simple-pay' ),
		'live_publishable_key' => esc_html__( 'Live Publishable Key', 'simple-pay' ),
		'live_secret_key'      => esc_html__( 'Live Secret Key', 'simple-pay' ),
	);
	...
	$settings->add(
		new Settings\Setting_Radio(
			array(
				...
				'options'     => array(
					'enabled'  => esc_html_x( 'Test Mode', 'setting label', 'simple-pay' ),
					'disabled' => esc_html_x( 'Live Mode', 'setting label', 'simple-pay' ),
				),
	...
	$settings->add(
		new Settings\Setting_Select(
			array(
				...
				'section'     => 'stripe',
				'subsection'  => 'account',
				'label'       => esc_html_x( 'Country', 'setting label', 'simple-pay' ),
	...
	$settings->add(
		new Settings\Setting_Select(
			array(
				...
				'section'     => 'stripe',
				'subsection'  => 'locale',
				'label'       => esc_html_x( 'Stripe Checkout', 'setting label', 'simple-pay' ),
