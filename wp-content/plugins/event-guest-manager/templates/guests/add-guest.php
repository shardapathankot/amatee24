...
<div class="wrap">
    ...
    <form class="add-guest" method="post" action="">
        <select name="event_id" required>
            ...
            <?php if ($query->have_posts()) : ?>
                <?php while ($query->have_posts()) : $query->the_post(); ?>
                    <option value="<?php the_ID(); ?>"><?php the_title(); ?> - <?php echo get_post_meta(get_the_ID(), 'start_datetime', true); ?></option>
                <?php endwhile; ?>
            <?php endif; ?>
        </select>
        ...
        <input type="number" name="guest_contact" min="0" placeholder="Guest contact" class="widefat" pattern="(\+\d{1,3})?[\s-]?(\(\d{1,3}\)|\d{1,3})[\s-]?(\d{1,4}[\s-]?){1,3}" title="Please enter a valid contact number" required>
        <input type="email" name="guest_email" placeholder="Guest email" class="widefat" required>
        <input type="text" name="table_number" placeholder="Table number" class="widefat" required>
        <input type="number" name="associate_guests" min="0" placeholder="Number of additional guests" class="widefat" required>
