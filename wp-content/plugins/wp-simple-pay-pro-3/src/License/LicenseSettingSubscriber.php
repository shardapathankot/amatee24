		...
		$settings->add(
			new Settings\Setting(
				array(
					...
					'section'    => 'general',
					'subsection' => 'license',
					'label'      => esc_html__( 'License Key', 'simple-pay' ),
					'output'     => function() {
		...
		switch ( $this->license->get_status() ) {
			case 'valid':
				return wp_kses(
					sprintf(
						...
						__( 'Your license level is %1$s.', 'simple-pay' ),
			...
			case 'expired':
				...
				return sprintf(
					...
					__( 'Your license key expired on %1$s', 'simple-pay' ),
			...
			case 'invalid':
			default:
				return esc_html__( 'Please enter a valid license key.', 'simple-pay' );
		}
	}
