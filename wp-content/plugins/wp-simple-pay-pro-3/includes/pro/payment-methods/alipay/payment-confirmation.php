...
namespace SimplePay\Pro\Payments\Payment_Methods\Alipay;
...
function validate_payment_confirmation_data() {
	...
	// Ensure we paid with Alipay.
	if ( ! isset( $_GET['source_type'] ) || 'alipay' !== $_GET['source_type'] ) {
		return;
	}
	...
	// Do nothing if the Intent is not from iDEAL.
	if ( 'alipay' !== $payment_intent->last_payment_error->payment_method->type ) {
		return;
	}
